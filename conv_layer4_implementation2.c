...
int main(int argc,char **argv) {
  ...
  if (rank == 0){
  	gettimeofday(&start, NULL);
	    int unit = images/(size);	
	    for (int coreindex = 0 ; coreindex <= (size-2); coreindex++){	
		    for(subindex = 0; subindex < unit; subindex++){	
			    ...
			    //read_image_pgm(in_image, imagename, 1280, 720);	
	        	    //printf("%s\n",imagename);		
	        	    imageindex = (coreindex*unit+subindex);
			    MPI_Send(&imageindex,1,MPI_INT,(coreindex+1),type,MPI_COMM_WORLD);	
		    }	
	    }
    	    
	    for(subindex = 0; subindex < unit; subindex++){
		imageindex = ( (size-1)*unit+subindex);
            	sprintf(imagename,"data/test%d.pgm",imageindex);   
                read_image_pgm(in_image, imagename, 1280, 720);   
                                                                                  		printf("%s\n",imagename);         
	...
	gettimeofday(&start, NULL);
	for(subindex = 0; subindex < (images/size);subindex++){
	 
		MPI_Recv(&imageindex,1,MPI_INT,0,type,MPI_COMM_WORLD,&status);
		sprintf(imagename,"data/test%d.pgm",imageindex);   
		read_image_pgm(in_image, imagename, 1280, 720);   
		printf("%s\n",imagename);
        	run_convolution_layer1(in_image, net_layer1, bias1, weight1);
